...
export default function ProfilePage({ params }: { params: { username: string } }) {
  ...
  if (!u) {
    return <div className="min-h-screen bg-[#080d14] text-white flex items-center justify-center">...</div>;
  }

  const isOwnProfile = user?.username === params.username;
  const xpPercent = Math.round(((u.xp || 0) / (u.xpToNextLevel || 1000)) * 100) || 0;
  const displayInitials = u.initials || u.username?.slice(0, 2).toUpperCase() || 'FV';
            ...
            {isOwnProfile ? (
              ...
              <button 
                ...
                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  isFollowing 
                    ? 'bg-white/10 text-white hover:bg-red-500/20 hover:text-red-400 border border-white/20 hover:border-red-500/50' 
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-5 -mt-16 mb-6">
            ...
            <div className="relative">
              <div className={`w-28 h-28 rounded-2xl bg-gradient-to-br from-emerald-400 to-green-600 ring-4 ring-offset-4 ring-offset-[#080d14] flex items-center justify-center overflow-hidden ${
                ...
                {u?.avatarUrl ? (
                  <img src={u.avatarUrl} alt="Avatar" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                ) : (
